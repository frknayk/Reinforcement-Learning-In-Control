...
# Nonlinear System Model is defined inside of 'gym.py' 
env = ENV(1,0.30)
...
# Creating CSV File
with open('log_output.csv','w') as f:
    writer = csv.writer(f)
...
for step in range(max_steps):
    ...
    # next_state, reward, done = env.step( np.array( [[u]] ), step )


    # print("Control Signal Without {0}".format(np.asscalar(action)))
    # print("Control signal : {0}\n***********************\n".format(u))
    ...
    with open('log_output.csv','a') as writeFile :
